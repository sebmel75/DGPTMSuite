 ...
 * <= 2024: Statische Daten aus DGPTM_FIN_Historical_Data
 * >= 2025: Live-Daten aus Zoho Books API
 *
 * Portiert von finanzbericht/class-report-builder.php mit Dependency Injection.
 *
 * @package DGPTM_Finanzen
 */
if (!defined('ABSPATH')) exit;

class DGPTM_FIN_Report_Builder {
    ...
    public function __construct(DGPTM_FIN_Zoho_Books $books) {
        $this->books = $books;
    }
    ...
    public function get_report(string $report, int $year): array {
        ...
        if ($year < self::DYNAMIC_START_YEAR) {
            $data = DGPTM_FIN_Historical_Data::get($report, $year);
            if (!$data) {
    ...
    public function get_available_years(string $report): array {
        $static = DGPTM_FIN_Historical_Data::available_years($report);
    ...
    private function build_dynamic(string $report, int $year): array {
        ...
        switch ($report) {
            case 'jahrestagung':
                return $this->build_jahrestagung($year, $period);
            case 'sachkundekurs':
                return $this->build_sachkundekurs($year, $period);
            case 'zeitschrift':
                return $this->build_zeitschrift($year, $period);
            default:
    ...
    private function build_jahrestagung(int $year, array $period): array {
        $income = $this->books->get_jt_income($period['start'], $period['end']);
        $expenses = $this->books->get_jt_expenses($period['start'], $period['end']);
    ...
    private function build_sachkundekurs(int $year, array $period): array {
        $income = $this->books->get_skk_income($period['start'], $period['end']);
        $expenses = $this->books->get_skk_expenses($period['start'], $period['end']);
    ...
    private function build_zeitschrift(int $year, array $period): array {
        $income = $this->books->get_zeitschrift_income($period['start'], $period['end']);
        $expenses = $this->books->get_zeitschrift_expenses($period['start'], $period['end']);
