...
class CCS_Plugin {
  ...
  function __construct() {
    ...
    add_action('admin_notices', function() {
      ...
      if (isset($_GET['page']) && sanitize_text_field($_GET['page'])==='ccs_reference') {
  ...
  function load_settings() {
    ...
    self::$settings_definitions = array(
      ...
      'shortcodes_by_non_admin_authors' => array(
        'default' => 'off',
        'module' => '',
        'tab' => '',
        'text' => 'Enable shortcodes for non-admin authors <span style="color:red">*</span>',
      ),
    );
  ...
  function setup_wp_filters() {
    ...
    /*---------------------------------------------
     *
     * Restrict shortcodes by constants, author capabilities, and settings
     *
     */
    ...
    $admin_shortcodes = [
      ...
      'load'  => ['CCS_Load', 'load'],
      'users' => ['CCS_User', 'users_shortcode'],
      'user'  => ['CCS_User', 'user_shortcode'],
    ];

    add_filter('the_content', function($content) use ($admin_shortcodes, $settings) {
      ...
      $is_admin = !empty($user_id) && in_array('administrator', $user->roles);

      if ((defined('DISALLOW_UNFILTERED_HTML') && DISALLOW_UNFILTERED_HTML)
        || !($is_admin
          || user_can($user_id, 'unfiltered_html') // Only super-admins on multisite
          || (isset( $settings['shortcodes_by_non_admin_authors'] ) &&
            $settings['shortcodes_by_non_admin_authors'] == 'on'
          )
        )
      ) {
        ...
        || !(user_can($user_id, 'edit_plugins') || user_can($user_id, 'edit_themes'))
      ) {

        remove_ccs_shortcode('load');
      }
    ...
    add_filter('the_content', function($content) use ($admin_shortcodes) {
      add_ccs_shortcode($admin_shortcodes); // Restore
      return $content;
    }, 999);
