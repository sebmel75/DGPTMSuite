...
if (!class_exists('DGPTM_Daten_Bearbeiten')) {
    ...
    class DGPTM_Daten_Bearbeiten {
        ...
        private function get_oauth_token() {
            ...
            $this->log('=== Token retrieval failed - Module status ===');
            $this->log('DGPTM_Mitgliedsantrag loaded: ' . (class_exists('DGPTM_Mitgliedsantrag') ? 'YES' : 'NO'));
            $this->log('DGPTM_Zoho_CRM_Hardened loaded: ' . (class_exists('DGPTM_Zoho_CRM_Hardened') ? 'YES' : 'NO'));
            $this->log('DGPTM_Zoho_Plugin loaded: ' . (class_exists('DGPTM_Zoho_Plugin') ? 'YES' : 'NO'));
        ...
        private function fetch_contact_from_crm($zoho_id, $token) {
            $url = 'https://www.zohoapis.eu/crm/v2/Contacts/' . $zoho_id;
            $this->log("Fetching contact: {$url}");
            ...
            $this->log("Response HTTP {$http_code}");
            ...
            $this->log("ERROR: Contact not found. Response: " . substr($body_raw, 0, 500));
            return false;
        }
