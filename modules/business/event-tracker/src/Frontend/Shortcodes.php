...
class Shortcodes {
	...
	public function event_tracker_shortcode( $atts ) {
		...
		if ( isset( $_POST['et_save_event'] ) ) {
			$save_result = $this->handle_frontend_save();

			// Check if save was successful and redirect to avoid form resubmission
			if ( strpos( $save_result, 'erfolgreich' ) !== false ) {
				// Success - redirect with success parameter
				$redirect_url = add_query_arg( 'et_saved', '1', remove_query_arg( 'et_saved' ) );
				wp_safe_redirect( $redirect_url );
				exit;
			} else {
				// Error - show message
				$notice_html = $save_result;
			}
		}

		// Show success message after redirect
		if ( isset( $_GET['et_saved'] ) && $_GET['et_saved'] === '1' ) {
			$notice_html = Helpers::notice( __( 'Veranstaltung erfolgreich gespeichert.', 'event-tracker' ), 'success' );
		}
