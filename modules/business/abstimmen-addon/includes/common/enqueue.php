<?php
/**
 * Asset Enqueue Handler (Consolidated)
 * Combines all asset loading logic in one place
 *
...
function dgptm_abstimmen_enqueue_admin_assets( $hook ) {
	...
	$version = defined( 'DGPTM_ABSTIMMEN_VERSION' ) ? DGPTM_ABSTIMMEN_VERSION : '4.0.0';
...
function dgptm_abstimmen_enqueue_frontend_assets() {
	...
	// Also provide dgptm_ajax for backwards compatibility
	wp_add_inline_script(
		'jquery',
		'window.dgptm_ajax=window.dgptm_ajax||{ajax_url:"' . esc_js( admin_url( 'admin-ajax.php' ) ) . '"};',
		'after'
	);
}
...
function dgptm_abstimmen_preload_assets() {
	// Only if DGPTM_ABSTIMMEN_URL is defined
