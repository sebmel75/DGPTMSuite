# GoCardless Billing Requests Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Neuer Shortcode `[gcl_formidable_new]` für Bankdaten-Änderung via GoCardless Billing Requests API, Dashboard-kompatibel, ohne Formidable Forms.

**Architecture:** Neue Datei `gocardless-billing-requests.php` im bestehenden GoCardless-Modul. API-Helper, 4 AJAX-Handler, Shortcode mit Inline-HTML, Event-Delegation für Dashboard. CRM-Sync bei Mandat-Änderung.

**Tech Stack:** WordPress 5.8+ / PHP 7.4+ / GoCardless API v2015-07-06 / Zoho CRM API v7 / jQuery Event-Delegation

**Spec:** `docs/superpowers/specs/2026-03-25-gcl-billing-requests-design.md`
...
|--------|-------|---------------|
| Create | `modules/payment/gocardless/gocardless-billing-requests.php` | Gesamte Billing Requests Logik: API-Helper, AJAX-Handler, Shortcode, Settings, CRM-Sync |
| Modify | `modules/payment/gocardless/gocardless-direct-debit-manager.php:349` | `require_once` der neuen Datei |

---

### Task 1: API-Helper + Konstanten

**Files:**
- Create: `modules/payment/gocardless/gocardless-billing-requests.php`

- [ ] **Step 1: Datei mit Header, Konstanten und API-Helper erstellen**
...
```bash
git add modules/payment/gocardless/gocardless-billing-requests.php
git commit -m "feat(gcl-br): add API helper, token reader, customer/mandate lookup"
```

---

### Task 2: AJAX-Handler (Status + Cancel + Change + New)

**Files:**
- Modify: `modules/payment/gocardless/gocardless-billing-requests.php`

- [ ] **Step 1: Alle 4 AJAX-Handler hinzufügen**
...
```bash
git add modules/payment/gocardless/gocardless-billing-requests.php
git commit -m "feat(gcl-br): add 4 AJAX handlers (status, change, new, cancel)"
```

---

### Task 3: Shortcode + Frontend (HTML/CSS/JS)

**Files:**
- Modify: `modules/payment/gocardless/gocardless-billing-requests.php`

- [ ] **Step 1: Shortcode-Registrierung, Assets, und Inline-JS hinzufügen**
...
```bash
git add modules/payment/gocardless/gocardless-billing-requests.php
git commit -m "feat(gcl-br): add shortcode [gcl_formidable_new] with inline UI/JS"
```

---

### Task 4: Settings-Sektion

**Files:**
- Modify: `modules/payment/gocardless/gocardless-billing-requests.php`

- [ ] **Step 1: Admin-Settings für redirect_url und exit_url hinzufügen**
...
```bash
git add modules/payment/gocardless/gocardless-billing-requests.php
git commit -m "feat(gcl-br): add admin settings for redirect/exit URLs"
```

---

### Task 5: Integration in Hauptdatei

**Files:**
- Modify: `modules/payment/gocardless/gocardless-direct-debit-manager.php:349`

- [ ] **Step 1: require_once vor der Instanziierung einfügen**

Zeile 349 (`new GCL_Formidable_Integration();`) ersetzen durch:
...
```bash
git add modules/payment/gocardless/gocardless-direct-debit-manager.php
git commit -m "feat(gcl): load billing-requests module"
```
...
DGPTM Suite → GoCardless aktivieren. Keine PHP-Fehler prüfen.

- [ ] **Step 2: Settings prüfen**
Admin → Einstellungen → GoCardless. Neue Sektion "Billing Requests" sichtbar mit Redirect/Exit URL.

- [ ] **Step 3: Shortcode testen**
Seite mit `[gcl_formidable_new]` aufrufen. Status wird per AJAX geladen.

- [ ] **Step 4: Dashboard-Tab testen**
Shortcode als Tab im Mitglieder-Dashboard konfigurieren. Tab wechseln → Status lädt.

- [ ] **Step 5: Bankverbindung ändern testen**
Button klicken → Bestätigung → Redirect zu GoCardless Hosted Flow.

- [ ] **Step 6: CRM-Sync prüfen**
Nach Rückkehr von GoCardless: Status laden → neue Mandat-ID → Zoho CRM `MandatID` aktualisiert.
