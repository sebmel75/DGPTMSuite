...
class DGPTM_Vimeo_Webinare {
    ...
    public function ajax_complete_webinar() {
        ...
        dgptm_log_info('Complete Webinar - User: ' . $user_id . ', Webinar: ' . $webinar_id, 'vimeo-webinare');
        ...
        if ($this->is_webinar_completed($user_id, $webinar_id)) {
            dgptm_log_info('Complete Webinar - Already completed', 'vimeo-webinare');
            wp_send_json_error(['message' => 'Webinar bereits abgeschlossen']);
        }

        // Create Fortbildung entry
        dgptm_log_info('Complete Webinar - Creating Fortbildung entry...', 'vimeo-webinare');
        ...
        dgptm_log_info('Complete Webinar - Fortbildung created: ' . $fortbildung_id, 'vimeo-webinare');

        /**
         * Fires after a Fortbildung entry has been created from a webinar completion.
         *
         * @param int $fortbildung_id The Fortbildung post ID.
         * @param int $user_id        The user ID.
         * @param int $webinar_id     The webinar post ID.
         */
        do_action('dgptm_fortbildung_created', $fortbildung_id, $user_id, $webinar_id);

        // Generate certificate
        dgptm_log_info('Complete Webinar - Generating certificate...', 'vimeo-webinare');
        ...
        if ($pdf_result && isset($pdf_result['url'])) {
            ...
            dgptm_log_info('Complete Webinar - Certificate generated: ' . $pdf_url, 'vimeo-webinare');

            // Add certificate as attachment to fortbildung post
            if ($attachment_id) {
                update_field('attachements', $attachment_id, $fortbildung_id);
                dgptm_log_verbose('Complete Webinar - Certificate attached to fortbildung: ' . $attachment_id, 'vimeo-webinare');
            }

            // Send email with certificate
            dgptm_log_info('Complete Webinar - Sending email...', 'vimeo-webinare');
            $mail_sent = $this->send_certificate_email($user_id, $webinar_id, $pdf_url);
            dgptm_log_info('Complete Webinar - Email sent: ' . ($mail_sent ? 'Yes' : 'No'), 'vimeo-webinare');
        } else {
    ...
    private function create_fortbildung_entry($user_id, $webinar_id) {
        dgptm_log_info('Create Fortbildung - Start: User ' . $user_id . ', Webinar ' . $webinar_id, 'vimeo-webinare');
        ...
        dgptm_log_info('Create Fortbildung - No existing entry, creating new...', 'vimeo-webinare');
        ...
        dgptm_log_info('Create Fortbildung - Post created: ' . $fortbildung_id, 'vimeo-webinare');
    ...
    private function generate_certificate_pdf($user_id, $webinar_id) {
        ...
        if (file_exists($pdf_path)) {
            ...
            if (!is_wp_error($attachment_id) && $attachment_id) {
                ...
                dgptm_log_verbose('Certificate PDF - Attachment created: ' . $attachment_id, 'vimeo-webinare');
            } else {
                dgptm_log_error('Certificate PDF - Failed to create attachment', 'vimeo-webinare');
            }
        }
