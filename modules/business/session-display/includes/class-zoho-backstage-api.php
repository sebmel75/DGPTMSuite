...
class DGPTM_Zoho_Backstage_API {
     ...
     * API: /portals/{portalId}/events/{eventId}/
     */
     ...
     * API: /portals/{portalId}/events/{eventId}/sessions?day={dayNumber}
     */
    public function get_sessions($event_id = null, $day = null) {
        ...
        // Wenn kein Tag angegeben, alle Tage abrufen
        if ($day === null) {
            return $this->get_all_days_sessions($event_id);
        }

        // Korrekter Endpoint mit day Parameter
        $endpoint = "/portals/{$this->portal_id}/events/{$event_id}/sessions?day={$day}";
        return $this->make_request($endpoint);
    }

    /**
     * Sessions für alle Tage abrufen
     */
    private function get_all_days_sessions($event_id) {
        // Event-Details abrufen um Anzahl der Tage zu ermitteln
        $event = $this->get_event($event_id);

        if (is_wp_error($event)) {
            error_log('DGPTM Session Display: Fehler beim Abrufen der Event-Details - ' . $event->get_error_message());
            // Fallback: Versuche Tag 1 und 2
            $days_to_fetch = [1, 2];
        } else {
            // Agenda-Tage ermitteln (z.B. aus Event-Daten)
            // Da wir die exakte Struktur nicht kennen, versuchen wir mehrere Tage
            $days_to_fetch = [1, 2, 3]; // Typischerweise 1-3 Tage
        }

        $all_sessions = [];

        foreach ($days_to_fetch as $day) {
            $day_sessions = $this->get_sessions($event_id, $day);

            if (!is_wp_error($day_sessions) && isset($day_sessions['sessions']) && is_array($day_sessions['sessions'])) {
                $all_sessions = array_merge($all_sessions, $day_sessions['sessions']);
            }
        }
     ...
     * API: /portals/{portalId}/events/{eventId}/sessions/{sessionId}
     */
     ...
     * API: /portals/{portalId}/events/{eventId}/tracks/{trackId}
     */
     ...
     * API: /portals/{portalId}/events/{eventId}/tracks/{trackId}
     */
    public function get_track($event_id, $track_id) {
        $endpoint = "/portals/{$this->portal_id}/events/{$event_id}/tracks/{$track_id}/";
        return $this->make_request($endpoint);
    }

    /**
     * Speakers abrufen
     * API: /portals/{portalId}/events/{eventId}/speakers/
     */
    ...
    public static function get_authorization_url() {
        ...
        // Ausgewählte Scopes abrufen
        $selected_scopes = get_option('dgptm_session_display_oauth_scopes', ['zohobackstage.agenda.READ']);

        // Wenn keine Scopes ausgewählt, Standard verwenden
        if (empty($selected_scopes)) {
            $selected_scopes = ['zohobackstage.agenda.READ'];
        }

        // Scopes zu komma-getrenntem String zusammenfügen
        $scope_string = implode(',', $selected_scopes);
